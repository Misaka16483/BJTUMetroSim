...
# The teacher workbook labels these arrays as whole-vehicle energy consumption.
# Their traction values reconcile with the whole-train line-current curve when
# interpreted as kJ per 0.1 s (for example 271 / 0.1 ~= 3277.5 A * 825 V).
ENERGY_CURVE_SAMPLE_SECONDS = 0.1
...
@dataclass(frozen=True)
class VehiclePowerDemand:
    """Gross DC-side power requested/generated by one train for one tick."""

    traction_power_request_kw: float
    regen_power_available_kw: float
    auxiliary_power_kw: float
    traction_mechanical_power_kw: float
    electric_brake_mechanical_power_kw: float

    @property
    def signed_gross_power_kw(self) -> float:
        return self.traction_power_request_kw + self.auxiliary_power_kw - self.regen_power_available_kw
...
class TractionDriveModel:
    ...
    def traction_energy_rate_kw(self, speed_mps: float) -> float:
        return (
            _interpolate(MOTOR_SPEED_RPM, ENERGY_CONSUMPTION_TRACTION_KW, self.motor_speed_rpm(speed_mps))
            / ENERGY_CURVE_SAMPLE_SECONDS
        )

    def brake_energy_rate_kw(self, speed_mps: float) -> float:
        return (
            _interpolate(MOTOR_SPEED_RPM, ENERGY_CONSUMPTION_BRAKE_KW, self.motor_speed_rpm(speed_mps))
            / ENERGY_CURVE_SAMPLE_SECONDS
        )

    def electrical_power_demand(
        self,
        ...
        *,
        ...
        """Map force demand to the teacher-curve DC-side power before network limits."""
        speed_mps = max(speed_mps, 0.0)
        traction_capacity_n = self.traction_capacity_n(speed_mps)
        electric_capacity_n = self.electric_brake_capacity_n(speed_mps)
        traction_ratio = (
            _clamp(demand.traction_force_n / traction_capacity_n, 0.0, 1.0)
            if traction_capacity_n > 1e-9
            else 0.0
        )
        regen_ratio = (
            _clamp(demand.candidate_electric_brake_force_n / electric_capacity_n, 0.0, 1.0)
            if electric_capacity_n > 1e-9
            else 0.0
        )
        return VehiclePowerDemand(
            traction_power_request_kw=self.traction_energy_rate_kw(speed_mps) * traction_ratio,
            regen_power_available_kw=self.brake_energy_rate_kw(speed_mps) * regen_ratio,
            auxiliary_power_kw=max(auxiliary_power_kw, 0.0),
            traction_mechanical_power_kw=demand.traction_force_n * speed_mps / 1000.0,
            electric_brake_mechanical_power_kw=demand.candidate_electric_brake_force_n * speed_mps / 1000.0,
        )
...
class SimpleVehicleModel:
    ...
    def step_with_forces(
        ...
        *,
        traction_force_n: float,
        brake_force_n: float,
        electric_brake_force_n: float | None = None,
        dt_s: float = 1.0,
        gradient_force_n: float = 0.0,
    ) -> TrainState:
        if dt_s <= 0:
        ...
        electric_brake_force_n = (
            brake_force_n if electric_brake_force_n is None else max(0.0, min(electric_brake_force_n, brake_force_n))
        )
        ...
        if traction_force_n > 0:
            ...
            traction_energy_kwh = energy_rate_kw * factor * dt_s / 3600.0
        if electric_brake_force_n > 0:
            ...
            elec_factor = min(electric_brake_force_n / max(electric_cap, 1.0), 1.0) if electric_cap > 0 else 0.0
            regen_energy_kwh = energy_rate_kw * elec_factor * dt_s / 3600.0
