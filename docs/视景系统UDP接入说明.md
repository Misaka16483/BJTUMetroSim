...
1. 按现场抓包发送 92 个信号状态和 40 个道岔状态；现有协议表可识别其中 77/29 个固定顺序，其余保留位先发送安全默认值。
2. 本车速度、发车倒计时、运行工况、牵引/制动百分比、EdgeID、区段内距离和方向。
3. 最多 128 列他车的位置、边号、方向和速度。
4. 显式小端编码，不依赖运行主机的字节序或 C 结构体对齐。
5. 前端设备管理器固定使用现场确认的变长数组布局；无他车时 UDP 载荷为 154 字节。
6. 100 ms 后台发布、状态统计、错误重试、CLI 单帧发送和 UDP 接收解码工具。
7. 后端 API 动态连接、断开、状态查询和协议 ID 到当前电子地图 ID 的覆盖配置。

尚需实验室实机确认：

1. Vision 1.3 的 `RunState` 字节到底仍使用 `0x11/0x12/0x13` 工况，还是现场程序已将其完全改作头灯控制。当前按协议表中的工况编码发送。
2. 当前电子地图和旧视景协议使用不同的信号/道岔编号。程序通过公里标和方向自动匹配；未匹配信号发送红灯，未匹配道岔发送定位，可在联调时通过 API 覆盖。
3. 现场抓包比旧表多出 15 个信号位和 11 个道岔位，其准确设备编号和排列顺序仍需现场补全；在确认前这些保留位同样发送红灯/定位。
...
| 本地源端口 | `8303` | 现场抓包确认，源端口和目标端口均为 8303 |
| 周期 | `100 ms` | 协议规定交换间隔 |
| 默认布局 | `compact` | 154 字节基准帧；每列他车增加 9 字节 |
